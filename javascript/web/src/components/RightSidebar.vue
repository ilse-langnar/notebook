...
.right-sidebar
    ...
    img( :src="ilse.require('settings.svg')" style="width: 20px;" @click="is_config_open = !is_config_open" )

    .flex( v-if="is_config_open" style="height: 50px; margin: auto; " )
        .loop.fitem( v-for="( item, index ) in list" :key="index" style="margin: auto; " )
            img.is-pulled-right( :src="ilse.require('x.svg')" style="width: 10px; cursor: pointer;  " alt="Delete"  @click="list.splice(index, 1)" )
            input.input( v-model="item.type" style="background: transparent; border: 0 !important;" )
        img.is-pulled-right( :src="ilse.require('plus.svg')" style="width: 30px; cursor: pointer;  " alt="Add Component" @click="add_component" )

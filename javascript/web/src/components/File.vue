...
.file( style="overflow: hidden;" )
    ...
    .flex
        img.is-pulled-left( v-if="!ilse.config.favorites || !(ilse.config.favorites.indexOf(file) !== -1)" :src="irequire.img('star.svg')" style="width: 20px; cursor: pointer; " @click="toggle_favorite(file)" :title="$t('favorite')" )
        img.is-pulled-left( v-if="ilse.config.favorites && ilse.config.favorites.indexOf(file) !== -1" :src="irequire.img('star-off.svg')" style="width: 20px; cursor: pointer; " @click="toggle_favorite(file)" :title="$t('unfavorite')" )
        input.is-size-4.centered.file-input( v-model="file" style="border: 0; " @keydown.enter="get_file" autofocus="autofocus" ref="input" )

        img.is-pulled-right( :src="irequire.img('trash.svg')" style="width: 20px; cursor: pointer; " @click="delete_file(file)" title="$t('delete_file')" )
